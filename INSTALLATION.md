...
```bash
docker pull ghcr.io/dubyte/dir2opds:v1.11.0
```
...
docker run \
  ...
  -v ./books:/books \
  --name dir2opds \
  ghcr.io/dubyte/dir2opds:v1.11.0
```

Use a specific [release](https://github.com/dubyte/dir2opds/releases) tag instead of `v1.11.0` if needed. Thanks to [rockavoldy](https://hub.docker.com/u/rockavoldy) for the run example.
...
```bash
podman pull ghcr.io/dubyte/dir2opds:v1.11.0
```
...
Add `-debug` for request logging, e.g. `... ghcr.io/dubyte/dir2opds:v1.11.0 /dir2opds -debug`.
